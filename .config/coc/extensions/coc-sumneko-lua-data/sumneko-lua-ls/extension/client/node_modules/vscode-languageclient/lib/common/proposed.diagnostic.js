        ...
        const openTabsHandler = () => {
            ...
            for (const group of vscode_1.window.tabGroups.all) {
                for (const tab of group.tabs) {
                    const kind = tab.kind;
                    if (kind instanceof vscode_1.TabKindText) {
                        this.open.add(kind.uri.toString());
                    }
                    else if (kind instanceof vscode_1.TabKindTextDiff) {
...
class DiagnosticRequestor {
    ...
    async pullAsync(document, version) {
        ...
        if (currentRequestState === undefined) {
            ...
            try {
                ...
                if (error instanceof features_1.LSPCancellationError && vscode_languageserver_protocol_1.Proposed.DiagnosticServerCancellationData.is(error.data) && error.data.retriggerRequest === false) {
    ...
    pullWorkspace() {
        this.pullWorkspaceAsync().then(() => {
            ...
            if (!(error instanceof features_1.LSPCancellationError) && !vscode_languageserver_protocol_1.Proposed.DiagnosticServerCancellationData.is(error.data)) {
        ...
        const result = {
            ...
            provideDiagnostics: (document, previousResultId, token) => {
                const provideDiagnostics = (document, previousResultId, token) => {
                    ...
                    return this.client.sendRequest(vscode_languageserver_protocol_1.Proposed.DocumentDiagnosticRequest.type, params, token).then(async (result) => {
        ...
        if (this.options.workspaceDiagnostics) {
            result.provideWorkspaceDiagnostics = (resultIds, token, resultReporter) => {
                ...
                const provideDiagnostics = (resultIds, token) => {
                    ...
                    const disposable = this.client.onProgress(vscode_languageserver_protocol_1.Proposed.WorkspaceDiagnosticRequest.partialResult, partialResultToken, async (partialResult) => {
                    ...
                    return this.client.sendRequest(vscode_languageserver_protocol_1.Proposed.WorkspaceDiagnosticRequest.type, params, token).then(async (result) => {
